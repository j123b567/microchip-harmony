    ...
    case WDRV_SECURITY_WEP_104:
        if ((gp_wdrv_cfg->securityKeyLen == 5) || (gp_wdrv_cfg->securityKeyLen == 13)) {
            ...
            for (i = 0; i < gp_wdrv_cfg->securityKeyLen * 2; ++i) {
                originalKeyChar = (i % 2 == 0) ? (gp_wdrv_cfg->securityKey[i / 2] >> 4) : (gp_wdrv_cfg->securityKey[i / 2] & 0x0F);
                asciiKey[i] = (originalKeyChar > 9) ? ('A' + originalKeyChar - 0x0A) : ('0' + originalKeyChar - 0x00);
            }
            gp_wdrv_cfg->securityKeyLen *= 2;
            memcpy(gp_wdrv_cfg->securityKey, asciiKey, gp_wdrv_cfg->securityKeyLen);
