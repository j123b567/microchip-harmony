...
static void IwconfigDisplayStatus(SYS_CMD_DEVICE_NODE *pCmdIO)
{
    ...
    for (i = 0; i < s_ctx.SSID.len; i++) {
        WDRV_DBG_INFORM_PRINT("%c", s_ctx.SSID.name[i]);
    }
    ...
    switch (s_ctx.SecurityType) {
        ...
        case WDRV_SECURITY_WPA_WITH_PASS_PHRASE:
            WDRV_DBG_INFORM_PRINT("WPA-PSK with pass phrase\r\n");
            break;
        case WDRV_SECURITY_WPA2_WITH_PASS_PHRASE:
            WDRV_DBG_INFORM_PRINT("WPA2-PSK with pass phrase\r\n");
            break;
...
static bool IwconfigSetSecurity(SYS_CMD_DEVICE_NODE *pCmdIO, int argc, char **argv)
{
    ...
    if ((argc >= 3) && (strcmp((char *)argv[2], "open") == 0)) {
        ...
        WDRV_DBG_INFORM_PRINT("7. wps pbc : iwconfig security pbc\r\n");
...
{
    if (WDRV_IsPowerOff()) {
        WDRV_DBG_INFORM_PRINT("MRF24WN is turned off - command cannot work\r\n");
        return false;
    }
