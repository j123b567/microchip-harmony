...
#elif WDRV_BOARD_TYPE == WDRV_BD_TYPE_CA70_XULT

static void __GPIO_OutLow_PIC32CZ_CA70_XULT(uint32_t channel, uint32_t bit_pos)
{
    SYS_PORTS_PinClear(PORTS_ID_0, channel, bit_pos);
}

static void __GPIO_OutHigh_PIC32CZ_CA70_XULT(uint32_t channel, uint32_t bit_pos)
{
    SYS_PORTS_PinSet(PORTS_ID_0, channel, bit_pos);
}
...
#elif WDRV_BOARD_TYPE == WDRV_BD_TYPE_CA70_XULT
    s_GPIOOutLow = GPIO_OUTLOW_FUNC(PIC32CZ_CA70_XULT);
    s_GPIOOutHigh = GPIO_OUTHIGH_FUNC(PIC32CZ_CA70_XULT);
#elif WDRV_BOARD_TYPE ==  WDRV_BD_TYPE_CUSTOM
