...
#include "driver/nvm/src/drv_nvm_static_local_pic32c.h"
...
uint8_t gDrvNVMEraseBuffer[DRV_NVM_INDEX_0+1][DRV_NVM_PAGE_SIZE];
</#if>
...
<#if CONFIG_USE_DRV_NVM_SYS_FS_REGISTER == true>
    ...
    SYS_FS_MEDIA_MANAGER_Register((SYS_MODULE_OBJ)DRV_NVM_INDEX_0, DRV_NVM_INDEX_0, &nvmMediaFunctions, SYS_FS_MEDIA_TYPE_NVM);
</#if>
...
void DRV_NVM_EraseWrite ( const DRV_HANDLE handle, DRV_NVM_COMMAND_HANDLE * commandHandle,
                          void* sourceBuffer, uint32_t writeBlockStart, uint32_t nWriteBlock )
{
    DRV_NVM_OBJECT *dObj = (DRV_NVM_OBJECT*)NULL;
    DRV_NVM_BUFFER_OBJECT * bufferObj;
<#if CONFIG_USE_3RDPARTY_RTOS>
...
<#if CONFIG_DRV_NVM_INTERRUPT_MODE == true>
    ...
    for (iEntry = 0; iEntry < DRV_NVM_BUFFER_OBJECT_NUMBER; iEntry ++)
    {
        if (gDrvNVMBufferObject[iEntry].inUse == false)
        {
			...
			bufferObj->flashMemPointer      = (uint32_t *)writeBlockStart;
...
void _DRV_NVM_WriteBufferObjProcess ( DRV_NVM_BUFFER_OBJECT* bufferObj )
{
	...
	_EFC_REGS->EEFC_FCR.w = (uint32_t)(EEFC_FCR_FCMD_WP | EEFC_FCR_FARG(page_number)| EEFC_FCR_FKEY_PASSWD);
...
DRV_NVM_COMMAND_HANDLE _DRV_NVM_BlockOperation ( uint8_t* source, uint32_t blockStart,
                    uint32_t nBlock, DRV_NVM_BUFFER_FLAGS flag, uint32_t blockSize )
{
    int iEntry;
    DRV_NVM_BUFFER_OBJECT * bufferObj;
    DRV_NVM_OBJECT *dObj = (DRV_NVM_OBJECT*)NULL;
...
<#if CONFIG_DRV_NVM_INTERRUPT_MODE == true>
            ...
            if (dObj->writeEraseQ == NULL)
            {
                ...
                {
                    if(DRV_NVM_BUFFER_FLAG_ERASE == flag)
                    {
