/*******************************************************************************
  ...
  Description:
    The NVM Driver provides a interface to access the NVM on the PIC32C
...
// *****************************************************************************
// *****************************************************************************
#include "driver/nvm/src/drv_nvm_local_pic32c.h"
...
uint8_t gDrvNVMEraseBuffer[DRV_NVM_INSTANCES_NUMBER][DRV_NVM_ERASE_BUFFER_SIZE];
...
// *****************************************************************************
/* Function:
    void _DRV_NVM_WriteBufferObjProcess ( DRV_NVM_OBJECT* dObj,
                                      ...
                                      DRV_NVM_BUFFER_OBJECT* bufferObj )
{
	...
	page_number = ((uint32_t) bufferObj->flashMemPointer - IFLASH_ADDR) / IFLASH_PAGE_SIZE;

	for (int i = 0; i < IFLASH_PAGE_SIZE; i += 4)
	{
		*((uint32_t *)( IFLASH_ADDR + ( page_number * IFLASH_PAGE_SIZE ) + i )) =	*((uint32_t *)( bufferObj->appDataPointer + i ));
	}
...
// *****************************************************************************
/* Function:
    void _DRV_NVM_EraseBufferObjProcess ( DRV_NVM_OBJECT* dObj,
                                          DRV_NVM_BUFFER_OBJECT* bufferObj )
...
// *****************************************************************************
/* Function:
    DRV_NVM_COMMAND_HANDLE _DRV_NVM_BlockOperation ( DRV_HANDLE handle, uint8_t* source,
                                            uint32_t blockStart, uint32_t nBlock,
                                            DRV_NVM_BUFFER_FLAGS flag, uint32_t blockSize )
...
// *****************************************************************************
/* Function:
    SYS_MODULE_OBJ DRV_NVM_Initialize ( const SYS_MODULE_INDEX drvIndex,
                                    ...
                                    const SYS_MODULE_INIT* const init )
{
    ...
    _DRV_NVM_RegisterWithSysFs (drvIndex, drvIndex, nvmMediaFunctions);
...
void DRV_NVM_Close ( const DRV_HANDLE handle )
{
     ...
     * dObj->writeEraseQ will not be NULL, else it will be NULL
     */
...
// *****************************************************************************
/* Function:
    void DRV_NVM_Read ( const DRV_HANDLE handle, DRV_NVM_COMMAND_HANDLE* commandHandle,
...
// *****************************************************************************
/* Function:
    void DRV_NVM_Write ( const DRV_HANDLE handle, DRV_NVM_COMMAND_HANDLE * commandHandle,
...
// **************************************************************************
/* Function:
    void DRV_NVM_Erase ( const DRV_HANDLE handle, DRV_NVM_COMMAND_HANDLE * commandHandle,
                         uint32_t blockStart, uint32_t nBlock )
...
// *****************************************************************************
/* Function:
    DRV_NVM_COMMAND_STATUS DRV_NVM_CommandStatus ( const DRV_HANDLE handle,
                                        ...
                                        const DRV_NVM_COMMAND_HANDLE commandHandle )
{
    ...
    /* Validate the client handle */
    _DRV_NVM_VALIDATE_EXPR((NULL == _DRV_NVM_ClientHandleValidate(handle)), (DRV_NVM_COMMAND_STATUS)DRV_NVM_COMMAND_HANDLE_INVALID);
...
void DRV_NVM_Tasks ( SYS_MODULE_OBJ object )
{
    ...
    if (bufferObj == NULL)
    {
         ...
         * request has closed the driver and the request has been removed
         * from the queue
		 * Disconnect the FRDY interrupt from the NVIC line to disable
		 * flash ready interrupt
		 */

			uint32_t ul_fmr = _EFC_REGS->EEFC_FMR.w;
			_EFC_REGS->EEFC_FMR.w = ( ul_fmr & (~EEFC_FMR_FRDY_Msk));
    ...
    switch (bufferObj->flag)
    {
        ...
        case DRV_NVM_BUFFER_FLAG_ERASE_WRITE:
            ...
            if ((bufferObj->nBlocksPending == 0) && (bufferObj->blockSize == 0))
            {
				...
				memcpy(dObj->eraseBuffer,(void*)(bufferObj->flashMemPointer),DRV_NVM_PAGE_SIZE);

                _DRV_NVM_EraseBufferObjProcess(dObj, bufferObj);
            }
            else
            {
                if (dObj->eraseWriteStep & DRV_NVM_ERASE_WRITE_STEP_ERASE_COMPLETE)
                {
                ...
                {
                    if (dObj->eraseWriteStep & DRV_NVM_ERASE_WRITE_STEP_WRITE_PAGE)
                    {
                        /* This is an on going operation */
                        bufferObj->flashMemPointer = (uint32_t *) ((uint32_t) bufferObj->flashMemPointer + DRV_NVM_ROW_SIZE);
                        bufferObj->appDataPointer += DRV_NVM_ROW_SIZE;
                        _DRV_NVM_WriteBufferObjProcess(dObj, bufferObj);
                    }
                    else
                    {
                        if (dObj->eraseWriteStep & DRV_NVM_ERASE_WRITE_STEP_ERASE_NEXT_PAGE)
                        {
...
// *****************************************************************************
/* Function:
    void DRV_NVM_EventHandlerSet ( const DRV_HANDLE handle, const void * eventHandler,
