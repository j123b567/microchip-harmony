...
static uint16_t             httpPersistTmo;             // the timeout after which a persistent connection is cloed, if no data is requested; seconds
...
bool TCPIP_HTTP_NET_DynamicFileInclude(HTTP_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT* varDcpt, const char* fileName)
{
    TCPIP_HTTP_NET_CONN* pHttpCon = (TCPIP_HTTP_NET_CONN*)connHandle;
    TCPIP_HTTP_CHUNK_DCPT* pChDcpt = (TCPIP_HTTP_CHUNK_DCPT*)varDcpt->dynContext;
