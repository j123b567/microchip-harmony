    ...
    switch(ori)
    {
        case GFX_ORIENTATION_0:
        {
            ...
            if(mirrored == GFX_TRUE)
                ...
                res.x = layer->rect.local.width - 1 - rect->x - rect->width;
            
            break;
        }
        case GFX_ORIENTATION_90:
        {
            /*res.x = rect->y + rect->height;
            res.width = rect->height;
            res.height = rect->width;
            
            if(mirrored == GFX_TRUE)
                res.y = layer->rect.local.width - 1 - rect->width - rect->x;
            else
                res.y = rect->x;*/
        ...
        {         
            ...
            if(mirrored == GFX_TRUE)
                ...
                res.x = layer->rect.local.width - 1 - rect->width - rect->x;
            
            res.y = layer->rect.local.height - 1 - rect->height - rect->y;*/
