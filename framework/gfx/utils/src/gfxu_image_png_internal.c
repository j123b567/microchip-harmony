                                     ...
                                     int32_t dest_y)
{
    ...
    error = lodepng_decode_memory(&image, (unsigned*) &img->width, (unsigned*) &img->height,
        img->header.dataAddress, pngsize, colorType, bitDepth);
    
    if (error == 0)
    {
        GFX_PixelBufferCreate(img->width,
            img->height,
            colorMode,
            image,
            &pixBuffer);
