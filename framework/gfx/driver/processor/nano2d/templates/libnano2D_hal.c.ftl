...
uint8_t __attribute__((coherent)) commandBuffer[CMD_BUFFER_SIZE] = {0};
                           ...
                           const GFX_DrawState* state)
{
    ...
    {
        n2d_color_t color = (n2d_color_t)state->maskValue;
...
{	
    ...
    context->memory.memset(&commandBuffer, 0, sizeof(commandBuffer));
    
    gpu_params.registerMemBase2D = 0xBF8EB000;
    gpu_params.contiguousBase = (uint32_t)commandBuffer;
