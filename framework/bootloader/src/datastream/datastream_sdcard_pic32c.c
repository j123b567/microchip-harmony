/*******************************************************************************
 Data Stream sdcard Source File

  File Name:
    datastream_sdcard_pic32c.c

  Summary:
 Data Stream SDCARD source
...
extern BOOTLOADER_DATA bootloaderData __attribute__((aligned(16)));
...
void DATASTREAM_Close(void)
{
    ...
    SYS_INT_VectorPrioritySet(DRV_TMR_INTERRUPT_VECTOR_IDX0, INT_PRIORITY_LEVEL0);

    SYS_INT_SourceDisable(HSMCI_IRQn);
    SYS_INT_VectorPrioritySet(HSMCI_IRQn, INT_PRIORITY_LEVEL0);

    SYS_INT_SourceDisable(XDMAC_IRQn);
    SYS_INT_VectorPrioritySet(XDMAC_IRQn, INT_PRIORITY_LEVEL7);
}
